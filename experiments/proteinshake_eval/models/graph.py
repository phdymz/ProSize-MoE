...
class GINConv(gnn.MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr=None, prompt=None):
        ...
        out = self.mlp[0](out, prompt = prompt, return_gate = False)
        out = self.mlp[1](out)
        out = self.mlp[2](out)
        out = self.mlp[3](out, prompt = prompt, return_gate = False)
...
class GNN(nn.Module):
    ...
    def forward(self, data):
        ...
        for layer in range(self.num_layers):
            output = self.gnns[layer](output, edge_index, edge_attr, prompt = prompt_atom)
